...
// `wt`: the exact worktree the holder session
...
/**
 * a global kill-switch banner for whenever an agent currently
 ...
 *
 * that local `.catch`-driven error state
 ...
 * the SAME error-state banner — reusing the earlier `stopFailed` UI/copy
 ...
 *
 * the earlier merge only ran one direction — nothing
 ...
 * a poll actually resolves — never on a rejection, preserving the earlier
 ...
 *
 * the earlier transition still requires a fulfilled
 ...
 * The transition check stays too — it still covers the case where
 * enabled is already false (e.g. the earlier fail-safe below never treats an
 * enabled poll as license to peek past a still-disabled state), and now
 * mostly acts as a harmless second path to the same clear. The
 ...
 *
 * The poll above is a `setInterval` firing every 3s
...
export function ComputerControlBanner() {
  ...
  // Two independent sources for the SAME error state
  // — a local one (this button's own call failed) and a
  ...
  // last CONFIRMED (successfully polled) server
  ...
  // from inside a fulfilled poll, so it shares the earlier fail-safe: a
  // rejected poll leaves it (and everything else) untouched.
  const lastServerStopFailedRef = useRef(false);
  // guards against a NEWER tick's results landing
  ...
  useEffect(() => {
    let alive = true;
    const tick = async () => {
      // Layer 1 — single-flight: if the previous tick's invokes
      ...
      // Layer 2 — generational guard (defense in depth): claim a
      ...
      try {
        ...
        // a transient invoke rejection must NEVER hide the
        ...
        // This applies to the earlier enabled poll too: a rejection is a no-op.
        ...
        if (persistResult.status === "fulfilled") {
          ...
          // Only a confirmed true -> false transition means "the
  ...
  const stop = useCallback(() => {
    // Single-flight — see `stopInFlightRef`'s doc. Guarded on
