export const zh = {
  common: {
    cancel: "取消",
    save: "保存",
    create: "创建",
    rename: "重命名",
    allow: "允许",
    deny: "拒绝",
    open: "打开",
    add: "添加",
    close: "关闭",
  },
  time: {
    justNow: "刚刚",
    mAgo: "{{n}} 分钟前",
    hAgo: "{{n}} 小时前",
    dAgo: "{{n}} 天前",
  },
  nav: {
    home: "工作区看板",
    repos_one: "{{count}} 个仓库",
    repos_other: "{{count}} 个仓库",
    threads: "issue",
    newThread: "新建 issue",
    deleteThread: "删除 issue",
    renameThread: "重命名 issue",
    renameWorkspace: "重命名工作区",
    deleteWorkspace: "删除工作区",
    collapseSidebar: "收起侧边栏",
    expandSidebar: "展开侧边栏",
    noThreads: "还没有 issue。新建一个开始。",
    createWorkspaceFirst: "先创建一个工作区。",
    repoMap: "仓库地图",
    needsYou: "待你处理",
    otherWorkspaceNeeds: "其他工作区有待办",
    newWorkspace: "新建工作区",
    noWorkspace: "无工作区",
    local: "本地 · 无服务端",
  },
  wsboard: {
    planning: "规划中",
  },
  palette: {
    search: "搜索",
    placeholder: "搜索 issue · 跳转 · 动作",
    empty: "无匹配",
    issue: "issue",
    go: "前往",
    action: "动作",
    board: "工作区看板",
    repos: "仓库地图",
    toggleSidebar: "切换侧边栏",
    theme: "切换主题",
    config: "有效配置",
  },
  config: {
    title: "有效配置",
    subtitle: "每个仓库实际生效的 skill 和 rule，以及它们来自哪一层。",
    skills: "Skills",
    rules: "Rules",
    layer_personal: "个人",
    layer_repo: "仓库",
    layer_team: "团队",
    "layer_weft-global": "Weft · 全局",
    "layer_weft-workspace": "Weft · 本工作区",
    loading: "读取配置…",
    empty: "未发现 skill 或 rule。",
    noRepos: "请先往这个工作区添加仓库。",
  },
  workspace: {
    tabRepos: "仓库",
    runningNow: "正在运行",
    threadsInFlight_other: "{{count}} 个进行中的 issue · 点击进入",
    threadsCount: "{{count}} 个 issue",
    noScope: "尚未划定范围",
    live: "{{count}} 个运行中",
    failing: "{{count}} 个失败",
    writtenByAnother: "其他 issue 也在写这个仓库",
    emptyTitleHas: "还没有 issue",
    emptyTitleNoWs: "无工作区",
    emptyBodyHas:
      "一个 issue 就是一条工作主线，可以跨多个仓库推进。新建后，Weft 会梳理改动范围、拆出任务，并主动提醒你确认关键决策。",
    emptyBodyNoWs: "先创建一个工作区。",
  },
  onboarding: {
    steps: ["欢迎", "工作区", "添加仓库", "依赖图", "第一个 issue", "跨仓拆解"],
    headerTitle: "欢迎使用 Weft",
    kicker: "新人引导",
    skip: "跳过",
    back: "上一步",
    start: "开始",
    next: "下一步",
    enterWorkspace: "进入工作区",
    heroSubtitle: "本地优先 · agent 驱动的多仓交付中心",
    heroBody: "描述需求后，Weft 会读取仓库地图、梳理改动范围、拆出任务，并持续推进到 PR。",
    workspaceTitle: "新建工作区",
    workspaceBody: "工作区是一份逻辑仓库清单，不是父仓，也不会拷贝你的代码。",
    addReposTitle: "添加本地仓库",
    addReposBody: "引用已有 Git 仓库，不拷贝代码。也可以跳过，稍后再添加。",
    moreRepos: "选更多本地仓…",
    repoEmpty: "添加一个或多个已有本地 Git 仓库。也可以跳过，稍后再添加。",
    addLocalRepo: "添加本地仓库",
    repoDuplicate: "这个仓库已经在列表里。",
    repoInvalid: "这些文件夹不是 Git 仓库：{{paths}}",
    repoNameLabel: "仓库名称",
    repoPathLabel: "仓库路径",
    removeRepo: "移除",
    graphTitle: "依赖图自动成形",
    graphBody: "从包清单连边。这张图装不进任何单个仓——它是后续跨仓拆解的依据。",
    firstIssueTitle: "新建第一个 issue",
    firstIssueBody: "选择类型，写下 Task。Lead 会读取仓库地图，先梳理改动范围。",
    scopeTitle: "梳理跨仓改动范围",
    scopeBody: "一个 Task 会并行铺开到多个仓库，并排好先后。Weft 先给出可审阅的拆分，运行前你随时校准。",
    scopeOrder: "api 先发契约 → web / mobile 并行接入",
    scopeApi: "优惠码契约 + 校验端点",
    scopeWeb: "Web 结算接入",
    scopeMobile: "移动端结算接入",
    title: "把一个 Task 拆成可交付的跨仓工作",
    body: "先从工作区开始。选入本地仓库后，Weft 会建立仓库地图；之后每个 issue 都能基于它梳理范围、拆出任务并持续验证。",
    stepRepos: "选择仓库",
    stepReposBody: "引用现有 git 仓库，不拷贝代码。",
    stepMap: "生成地图",
    stepMapBody: "记录每个仓库的职责、技术栈和依赖关系。",
    stepIssue: "拆解任务",
    stepIssueBody: "Lead 根据 issue 判断哪些仓库需要改动、哪些只作为参考。",
    localOnly: "本地优先 · 无服务端 · 不改仓库配置",
    footerHint: "现在只需要完成第一步：创建一个工作区。",
    primaryCta: "创建第一个工作区",
    secondaryCta: "稍后再说",
    previewTitle: "从仓库地图到执行范围",
    previewHint: "这是 Weft 的核心工作方式",
    scopePreview: "建议写入范围",
  },
  thread: {
    directionsSub_other: "{{count}} 个任务 · 并行执行，各自划定到自己的仓库",
    console: "控制台",
    tabBoard: "看板",
    runChecks: "运行该任务的检查（lint / type / test）",
    noChecks: "未推断出检查项",
    acceptPass: "验收通过",
    review: "发起 review",
    reviewTip: "进入会话并运行全局 review skill",
    handle: "处理",
    viewChanges: "查看改动",
    openSession: "打开会话",
    noWriteCopy: "尚未建副本",
    testsPending: "检查待跑",
    testsProgress: "检查 {{passed}}/{{count}}",
    provenance: "详情",
    provenanceTip: "仓库、工作分支与检查结果",
    copyBranch: "复制分支",
    copyPath: "复制工作副本路径",
    browseFiles: "浏览文件",
    copiesCount: "{{count}} 个副本",
    colRunning: "进行中",
    colNeeds: "待你处理",
    statusPlanning: "规划中",
    statusBuilding: "实施中",
    colReview: "待审阅",
    colDone: "已完成",
    colEmpty: "—",
    dropHere: "拖到此处移动",
    setStatus: "设置状态",
    renameTask: "重命名任务",
    deleteWorktree: "删除 worktree",
    deleteWorktreeTitle: "删除 worktree？",
    deleteWorktreeBody:
      "将删除位于 {{path}} 的 worktree 目录。分支 {{branch}} 会保留。worktree 中任何未提交的改动都会丢失。",
    deleteWorktreeConfirm: "删除",
    worktreeRemoved: "已删除",
    discussTitle: "和 lead 聊聊来规划这个 issue",
    discussBody:
      "在右侧的对话里描述你想要什么。lead 会读取你的仓库、提议如何拆分工作，你在这里确认成任务。",
  },
  lead: {
    title: "Lead",
    start: "启动 lead",
    compose: "给 lead 发消息…",
    attachFile: "@ 文件",
    autoDispatchHint: "Lead 自动拆解任务 · Weft 不设审批关卡",
    consoleSubtitle: "只读查看工作区仓库，不写代码；负责规划、梳理范围，并推动任务交付。",
    issueProgress: "本 issue 进度",
    subtasks: "任务",
    runningCount: "{{count}} 进行",
    needsCount: "{{count}} 待你",
    reviewCount: "{{count}} 评审",
    subtaskSnapshot: "任务快照",
    noSubtasksYet: "Lead 梳理范围后，任务会出现在这里。",
    dispatchTitle: "已创建 {{count}} 个任务",
    dispatchSubtitle: "按依赖顺序推进",
    waitingScope: "等待 Lead 梳理范围",
    waitingScopeBody: "描述任务后，Lead 会读取仓库地图，并给出可审阅的拆分卡。",
    send: "发送",
    proposalReady_other: "建议 {{count}} 个任务",
    proposalResolved_other: "已处理 {{count}} 个任务",
    proposalWithdrawn: "已撤回任务：{{rationale}}",
    reviewCreate: "查看并创建",
    starting: "正在启动 lead…",
    stopped: "lead 已停止。",
    restart: "重启 lead",
    viewChat: "对话",
    transcriptEmpty: "还没有产出。",
    taskEmptyTitle: "拆出下一步行动",
    taskEmptyBody:
      "告诉 Lead 目标和约束；它会读取仓库地图、梳理范围，并拆出清晰任务。",
    onboardingCueEyebrow: "先理清方向",
    onboardingCueBody:
      "说清想达成什么、有哪些限制。Lead 会梳理实施路径、拆出可执行任务，并在这里持续展示进展。",
    suggestionLabel: "可以试试",
    suggestionPlan: "梳理实施路径",
    suggestionQueue: "拆出下一步行动",
    suggestionTask: "补充验收标准",
    suggestionDescribe: "描述目标",
    suggestionAttach: "添加文件或图片",
    suggestionSlash: "输入 / 调用命令",
    suggestionImportRepo: "导入本地仓库",
    suggestionCloneRepo: "从 Git URL 克隆",
    suggestionCreateRepo: "新建空仓库",
    imageAttachment: "图片 {{count}}",
    pastedImage: "粘贴的图片 {{count}}",
    repoGuideTitle: "先接入一个仓库",
    repoGuideBody:
      "这个工作区还没有本地 git 仓库。这里添加后，Weft 会刷新仓库地图，并把结果回灌到这段对话。",
    repoGuideStepChoose: "选择已有目录、新建空仓库，或从 Git URL 克隆。",
    repoGuideStepMap: "Weft 会为仓库画像，并更新用于判断范围的仓库地图。",
    repoGuideStepReturn: "结果会回到聊天里，lead 可以基于更新后的工作区继续推进。",
    working: "处理中…",
    loading: "加载中",
    historyLoadError: "对话加载失败。",
    retryHistory: "重试",
    stop: "停止",
    interrupted: "已中断",
    errored: "本回合出错",
    terminalErrorBeforeOutput: "Agent 在产出内容前出错，本回合已结束。",
    terminalAgentNotFound: "未找到 Agent「{{tool}}」:请确认已安装并在 PATH 中,或在设置中为它配置命令路径。",
    terminalInterruptedBeforeOutput: "Agent 在产出内容前被中断，本回合已结束。",
    busyHint: "正在回复 · 此时发送会排队，回合结束自动送出",
    attachFiles: "添加文件",
    removeAttachment: "移除 {{name}}",
    copyMessage: "复制",
    copied: "已复制",
    queueFull: "队列已满 (5/5)，先删一条排队消息",
    queueEdit: "编辑",
    queueDelete: "删除",
    queueDrag: "拖动重排",
    queueAttachmentOnly: "{{count}} 张图片",
    autoReviewDirective: "review 结束后，直接修复发现的问题并重新跑检查自验，然后简要汇报。",
    receiptDelivered: "已送达",
    receiptConsumed: "Agent 已开始处理",
    sendFailedGeneric: "发送失败：{{reason}}",
    sendFailedUnknown: "未知错误",
    engineRouteBlocked: "引擎路由已阻断：{{reason}}",
  },
  scope: {
    addReposFirst: "请先往这个工作区添加仓库。",
    title: "审阅范围",
    rationale: "lead 的拆分思路",
    inputTask: "输入的 Task",
    inferred: "Lead 拆成这些 · 可纠正",
    unknownRepo: "不在这个工作区——创建时会跳过",
    close: "关闭",
    confirm_other: "创建 {{count}} 个任务",
    confirmWithPlan_other: "确认方案并创建 {{count}} 个任务",
    confirmFailed: "创建任务失败，请重试。",
    confirming: "创建中…",
    baseBranch: "基线分支",
    baseBranchHint: "从该分支拉出工作分支（留空 = 仓库默认分支）",
    // 中文的 Intl.PluralRules 对任何数量都取 "other"，所以这里只有 _other 一种形态，
    // 措辞必须对单个和多个依赖都自然（{{name}} 是逗号拼接后的名字列表）。
    dependsOn_other: "等待「{{name}}」先落地",
    dependsOnHint_other: "「{{name}}」合并之前，这个任务无法合并",
    engine: "引擎",
    engineAutomatic: "自动 · {{tool}}",
    engineManual: "已 pin · {{tool}}",
    engineLegacy: "默认 · {{tool}}",
    engineBlocked: "自动路由已阻塞",
    engineReason: "{{reason}}",
    batchEngine: "本次批量确认使用",
    batchEngineAutomatic: "自动路由",
    batchEngineHint:
      "保持自动路由，或为本批尚未启动的任务 pin 一个已安装引擎；已有 pin 或已启动的任务会保留当前引擎。",
    batchEnginePinned:
      "本批尚未启动的任务已 pin：{{tool}}；已有 pin 或已启动的任务会保留当前引擎。",
    routeReason: {
      automatic_disabled: "自动路由已关闭",
      manual_pin: "手动选择优先",
      normal_preference: "普通工作优先 Codex",
      deep_preference: "深度工作优先 Claude",
      preferred_warning: "首选引擎接近额度上限，改用当前更稳妥的可用选项",
      preferred_unavailable: "首选引擎不可用",
      quota_unknown: "暂时没有额度状态",
      legacy_fallback: "自动候选不可用，沿用默认工具",
      no_automatic_candidate: "没有安装可用的自动候选引擎",
      automatic_candidate_unavailable: "自动候选不可用或已耗尽",
      both_automatic_candidates_exceeded: "Codex 和 Claude 的额度都已耗尽",
      invalid_manual_tool: "手动引擎选择无效",
      manual_tool_unavailable: "手动引擎选择不可用",
    },
    engineRouteBlockedHint: "请选择手动引擎，或让一个可用的自动候选恢复后再重试。",
    planSummary: "方案摘要",
    readOnlyPropagationNote: "批准后，这些 worker 的只读检查（如 git status）也会自动放行，无需再问——随时可在看板上撤销。",
  },
  notify: {
    needsTitle: "待你处理",
    needsBody_other: "{{count}} 项新增待办",
    reviewTitle: "可以 review 了",
    reviewBody_other: "{{count}} 个任务进入 review",
    quotaTitle: "进程额度已满",
    quotaBody_other: "已暂停新会话和新回合",
  },
  processQuota: {
    warningTitle: "进程额度接近上限",
    warningBody: "当前仍可启动新工作；达到 {{degraded}}% 后会自动暂停。",
    degradedTitle: "已暂停新会话和新回合",
    degradedBody: "正在运行的任务继续；回落到 {{recovery}}% 后自动恢复。",
    usage: "{{count}} / {{limit}} · {{percent}}%",
    countOnly: "{{count}} 个进程",
    openDetails: "查看详情",
    warningToast: "Weft 使用的进程额度即将达到系统上限。",
    degradedToast: "进程额度不足，已暂停新会话和新回合。",
    recoveredToast: "进程额度已恢复，新会话和新回合可以继续。",
    blockedToast: "进程额度仍处于降级状态。消息已保留，请在恢复后重试。",
  },
  needs: {
    acpPermissionRequired: "需要授权",
    title: "待你处理",
    subtitle: "只有你能回答的审批与提问",
    wantsPermission: "请求权限",
    question: "需要回答",
    planApproval: "批准计划",
    scopeApproval: "批准范围",
    repoAction: "仓库操作",
    prTrackingRetry: "PR 跟踪需要重试",
    openQueue: "查看队列",
    more: "更多",
    always: "始终允许",
    allowTitle: "仅允许这一次请求",
    alwaysTitle: "以后自动允许相同的命令",
    denyTitle: "拒绝这次请求，交回 agent 自行决定",
    fullAccess: "完全访问",
    fullAccessTitle: "自动批准该任务的所有请求",
    releaseReadOnly: "放行本会话只读",
    releaseReadOnlyTitle: "以后自动放行本会话内的全部只读请求（写入、网络/凭证类请求仍会询问）",
    emptyTitle: "暂无待办",
    emptyBody:
      "当 agent 遇到只有你能拍板的决定时，会在这里发问。你回答一次，答复会直接回到它的收件箱，你无需去翻找会话。",
    answerPlaceholder: "回复 {{name}}…",
    openDirection: "打开这个任务",
    approvePlan: "批准计划",
    reviewScope: "审核范围",
    reviewRepoActions: "审核仓库操作",
    openPullRequest: "打开 Pull Request",
    pullRequestNumber: "PR #{{number}}",
    wantsToWrite: "想修改",
    approveRun: "批准并运行",
    approveRunTitle: "创建 worktree 并拉起 worker",
    denyWriteTitle: "拒绝这次改动;通知 lead",
    runWith: "执行工具",
    riskReadOnly: "只读",
    riskReadOnlyTitle: "只查看状态，不做改动",
    riskWrite: "写入",
    riskWriteTitle: "可能新建、修改或删除本地内容",
    riskNetworkOrCredential: "网络/凭证",
    riskNetworkOrCredentialTitle: "会离开本机或接触凭证 —— 请仔细核对",
    riskUnknown: "风险未知",
    riskUnknownTitle: "无法判定风险，批准前请仔细查看参数",
    screenshotPreview: "最新截图",
    detailTruncated: "……已截断 {{n}} 个字符",
    retryTracking: "重试",
    retryTrackingStarted: "已请求重试，Weft 会尽快重新检查。",
    retryTrackingFailed: "重试失败，请稍后再试。",
    answerFailed: "回答发送失败，草稿已保留，请重试。",
  },
  tool: {
    input: "输入",
    output: "输出",
    showMore: "展开剩余 {{n}} 行",
    showLess: "收起",
    screenshot: "截图 {{count}}",
  },
  session: {
    back: "返回看板",
    lead: "Lead",
    resume: "恢复",
    resumeReady: "恢复此会话",
    starting: "启动中…",
    clickToInteract: "点击进入交互",
    message: "给这个 agent 发消息…",
    chatStopped: "worker 引擎已停止 · 发送消息会自动续上",
    toolEditing: "正在编辑",
    toolReading: "正在读取",
    toolSearching: "正在搜索",
    toolRunning: "正在运行",
    toolSyncing: "正在同步",
    toolOrganizing: "正在整理",
    toolThinking: "正在思考",
    toolDelegating: "正在委派",
    toolCalling: "正在调用",
    toolEdited: "已编辑",
    toolRead: "已读取",
    toolSearched: "已搜索",
    toolRan: "已运行",
    toolSynced: "已同步",
    toolOrganized: "已整理",
    toolThought: "已思考",
    toolDelegated: "已委派",
    toolCalled: "已调用",
    resumeMenu: "恢复…",
    interacting: "交互中",
    backToReadonly: "返回只读",
    promptOk: "确定",
    promptCancel: "取消",
    rewindTip: "回退到这条消息之前",
    rewindTitle: "回退对话？",
    rewindBody: "这条消息之后的对话将被移除，agent 将不再记得它们；这条消息的文本会回填到输入框，可编辑后重发。",
    acpSessionOpenFailed: "无法打开 agent 会话。请确认 agent 已安装后重试。",
    ompSessionMissing: "找不到本地会话文件（agent 可能已清理）。请直接发新消息，而不是回退。",
    ompUserMissing: "在 agent 会话文件里找不到这条消息，请换一条再试。",
    ompScanTruncated: "agent 会话目录里的文件太多，未能完整搜索，因此没能定位到这个会话。清理旧的会话文件后即可搜到。",
    rewindConfirm: "回退",
    rewindModeConversation: "仅回退对话",
    rewindModeConversationDesc:
      "这条消息之后的对话将被移除，agent 不再记得它们；该消息的文本会回填到输入框。",
    rewindModeCode: "仅回退代码",
    rewindModeCodeDesc:
      "工作区文件恢复到这条消息之前的状态，未提交的改动会被覆盖（会保留安全快照）。",
    rewindModeBoth: "对话和代码",
    rewindModeBothDesc: "同时回退对话并恢复工作区代码。",
    rewindCodeTitle: "回退代码？",
    rewindCodeBody:
      "工作区文件将恢复到这条消息之前的状态，当前未提交的改动会被覆盖（会保留一份安全快照）。",
    rewindCodeConfirm: "回退代码",
    rewindBothTitle: "回退对话和代码？",
    rewindBothBody:
      "这条消息之后的对话将被移除，agent 不再记得它们（该消息文本回填到输入框）；工作区文件也将恢复到这条消息之前的状态，未提交的改动会被覆盖（会保留一份安全快照）。",
    rewindBothConfirm: "全部回退",
    rewoundMarker: "已从此处回退",
    rewindPickerTitle: "回退到之前的消息",
    rewindPickerEmpty: "没有可回退的消息。",
    switchScopeLead: "只影响这个 issue 的 lead；已经在跑的 worker 用各自的引擎，不受影响，也不会动新 issue 的默认引擎。",
    switchScopeWorker: "只影响这个任务的 worker；issue 的 lead 和其它任务各用各的引擎，不受影响，也不会动新任务的默认引擎。",
    switchTitleLead: "切换 lead 的引擎",
    switchTitleWorker: "切换该任务的引擎",
    switchToolLabel: "引擎",
    switchModelLabel: "模型覆盖",
    switchModelHint: "以 --model 传给 CLI；留空则用它自己配置的默认模型。",
    switchModelUnsupported: "{{tool}} 暂不支持模型覆盖。",
    switchModelPlaceholder: "例如 opus、gpt-5.5-high…",
    switchBody: "会清空这段对话的原生会话——新引擎从零开始。为了不让 {{tool}} 从零认知起步，下一条消息会附带一份最近对话的摘要——也就是说，这段内容现在也会发给 {{tool}}，不再只有原来那个引擎能看到。",
    switchReloadBody: "会用全新原生会话重启这个引擎——用来让你刚改的 CLI 配置（比如它的默认模型）生效，或者救回一个卡住的引擎。",
    switchConfirm: "切换",
    switchReloadConfirm: "重新加载",
    switchedToast: "已切换：{{from}} → {{to}}",
    switchReloadedToast: "{{tool}} 已重新加载",
    switchInProgress: "引擎正在因额度达限切换，请稍后重试。",
    switchFailed: "这次切换没成功——会话还在原来的引擎上，也没有影响到它手头的任何事。请重试。",
    switchFailedInterrupted: "这次切换没成功——会话还在原来的引擎上，但正在进行的那轮已经被中断。请重试。",
    engineSwitchedMarker: "引擎已切换：{{from}} → {{to}}",
    engineReloadedMarker: "引擎已重新加载",
    engineModelCleared: "无覆盖",
    engineSwitchedQuotaReason: "自动 · 额度达限",
    engineRouteMarker: "已自动路由到 {{tool}}",
    engineRouteManualMarker: "已 pin 到 {{tool}}",
    engineRouteBlockedMarker: "自动路由已阻塞",
    engineRouteBlockedTransition: "{{from}} → {{to}}",
    engineRouteQuotaBasis: "额度：{{status}}",
    engineRouteBlockedHint: "请选择手动引擎或调整全局路由设置，然后重试。",
    quotaFailoverFailedMarker: "自动 fail-over 到 {{to}} 失败——仍在 {{from}} 上",
    autoMergeSucceeded: "已自动合并 {{abbrev}} #{{number}} 到 {{base}}",
    autoMergeFailed: "{{abbrev}} #{{number}} 自动合并失败",
    autoMergeStateOpen: "仍是 open，未合并",
    autoMergeStateMerged: "已确认 merged",
    autoMergeStateClosed: "已关闭，未合并",
    autoMergeStateUnknown: "暂时无法确认当前状态",
    autoMergeAttemptsExhausted: "已连续失败 {{count}} 次，已停止自动重试——push 一个新 commit 即可重新尝试",
  },
  diff: {
    tab: "改动",
    empty: "还没有改动。",
    emptyTarget: "与目标分支暂无差异。",
    modeWorktree: "工作区",
    modeTarget: "目标分支",
    compareAgainst: "对比",
    refreshTarget: "从远端刷新",
    filesChanged_other: "{{count}} 个文件",
    untrackedOnly: "目前只有新增（未跟踪）文件。",
    ask: "标注这个文件",
    askLine: "点击这一行标注",
    askTitle: "标注",
    askLineRef: "对第 {{side}}{{n}} 行",
    askFileRef: "对这个文件",
    askPlaceholder: "请求更改",
    askHint: "{{key}} 发送 · Esc 取消",
    askCancel: "取消",
    askSubmit: "发送",
    askContext: "关于 `{{path}}` 这处改动：",
    askContextLine: "关于 `{{path}}`（第 {{side}}{{n}} 行）这处改动：",
  },
  files: {
    tab: "文件",
    empty: "该 worktree 为空。",
    loading: "加载文件中…",
    treeLabel: "文件树",
    expandFolder: "展开文件夹",
    collapseFolder: "收起文件夹",
    truncated: "目录太大，部分条目已隐藏。",
    refresh: "刷新文件树",
  },
  sessionInfo: {
    title: "会话信息",
    used: "已用 {{pct}}%",
    pending: "首条消息后可见",
    noMcp: "本会话没有用户配置的 MCP server",
    noSkills: "未启用 skill",
    skillsInjected: "注入",
    skillsInjectedHint: "Weft 为该工作区启用的 skill",
    skillsDiscovered: "运行时发现",
    skillsDiscoveredHint: "引擎在本次会话目录里实际发现的 skill",
    noEngineSkills: "本次会话未发现 skill",
    reload: "重载会话",
    reloadHint: "重新扫描本会话的 skill / MCP",
    context: "Context",
    tokens: "tokens",
    reasoning: "思考强度",
    skills: "Skills",
    mcp: "MCP",
    tools_other: "{{count}} 个 tool",
    subtasks: "任务",
    showMore_other: "展开剩余 {{count}} 个",
    showLess: "收起",
    engine: "引擎",
    engineUnknown: "未知",
    switchEngine: "切换…",
    // issue #103:本会话的只读自动放行状态（只存内存，重启即失效；恒不覆盖写入
    // 或网络/凭证类请求）。
    access: "授权",
    readOnlyIssueTrusted: "只读已放行（整个 issue）",
    readOnlySessionTrusted: "只读已放行（本会话）",
    revokeReadOnlyTrust: "撤销",
  },
  observe: {
    answerPlaceholder: "输入回复，回车发送",
  },
  bus: {
    close: "关闭",
  },
  resume: {
    copyCommand: "复制恢复命令",
    copied: "已复制到剪贴板",
    openInCodex: "在 Codex 中打开",
  },
  repomap: {
    subtitle: "每个仓库是什么，以及它们如何相互依赖",
    coreDependents_other: "核心 · {{count}} 个依赖方",
    rippleTitle: "{{count}} 个仓库依赖它 —— 改动会波及",
    reanalyze: "分析依赖",
    reanalyzeHint: "让仓库分析助手重新分析：为每个仓库判定层级并推断跨仓依赖（只读）",
    reanalyzeAllMissing: "无法重新分析：所有仓库的本地检出都不存在（已移动或删除）。",
    reanalyzeUnanalyzed_one: "{{count}} 个仓库未能分析：{{names}}",
    reanalyzeUnanalyzed_other: "{{count}} 个仓库未能分析：{{names}}",
    reanalyzeMessage: "请重新分析仓库间的依赖关系。",
    detailTab: "详情",
    curatorTitle: "仓库分析助手",
    curatorLoading: "正在打开仓库分析助手…",
    curatorCompose: "给仓库分析助手发消息…",
    curatorEmpty: "和仓库分析助手聊聊仓库间的依赖关系",
    deleteRepo: "从工作区移除",
    deleteRepoTitle: "移除 {{name}}？",
    deleteRepoBody:
      "将该仓库从工作区移除——包括它的画像、绑定到它的任务及其工作树。你磁盘上的实际仓库不会被删除。",
    deleteRepoConfirm: "移除仓库",
    deleting: "移除中…",
    dependsOn: "依赖",
    usedBy: "被依赖",
    via: "经由 {{via}}",
    legend: "拖动画布 · 点击仓库查看资料",
    oneLine: "一句话职责",
    stack: "技术栈",
    tier: "层级",
    category: "角色",
    domains: "领域",
    components: "内部组件",
    notAnalyzed: "尚未分析",
    analysisQueued: "排队等待分析…",
    analysisRunning: "分析中…",
    analysisFailed: "分析失败",
    analysisErrorCheckoutMissing: "本地检出未找到",
    analysisErrorUnsupportedEngine: "仓库分析暂不支持 {{tool}}，请为 curator 选择其他引擎。",
    retryAnalysis: "重试",
    viewOverview: "概览",
    viewExpanded: "展开",
    pkgCount_other: "{{count}} 个包",
    profiledAt: "画像提交",
    noDeps: "无依赖",
    noUsedBy: "暂无仓库依赖它",
    none: "无",
    summaryPlaceholder: "一句话:这个仓库是做什么的?",
    editSummaryTitle: "编辑描述",
    editHint: "点击编辑 —— 你的描述优先于自动推断",
    addSummary: "添加一句话描述",
    yours: "你的",
    selectRepo: "选择一个仓库",
    selectRepoBody: "右侧会显示职责、接口、依赖和画像来源。",
    emptyTitle: "还没有仓库画像",
    emptyBody:
      "往这个工作区添加仓库。只读 agent 会为每个仓库分类——层级、技术栈、摘要——并连成一张依赖图，供 lead 拆分工作。",
    tier_frontend: "前端",
    tier_backend: "服务端",
    tier_other: "未分类",
    layer_client: "客户端",
    layer_gateway: "网关",
    layer_biz: "Biz 业务",
    layer_core: "Core 核心",
    layer_support: "支撑服务",
    layer_foundation: "基础层",
    layer_other: "未分类",
    zoomIn: "放大",
    zoomOut: "缩小",
    fit: "适应视图",
    chatTab: "对话",
    mapTab: "地图",
    mapEmpty: "运行分析后生成依赖地图",
  },
  inspect: {
    label: "检视 —— 显示、复制路径",
    reveal: "显示工作副本",
    copyPath: "复制路径",
    branchLabel: "分支",
    sessionLabel: "会话",
  },
  fileLink: {
    openWith: "用默认应用打开",
    openFolder: "打开所在文件夹",
    copyPath: "复制路径",
    notFound: "打不开 —— 文件不存在",
    openFailed: "打不开这个文件",
  },
  dialog: {
    newWorkspaceTitle: "新建工作区",
    workspaceName: "名称",
    workspaceNamePlaceholder: "工作区名称",
    createWorkspace: "创建工作区",
    deleteWorkspaceTitle: "删除工作区？",
    deleteWorkspaceBody:
      "将删除工作区“{{name}}”及其 issue、仓库引用、任务状态和 Weft 创建的 worktree。你的原始本地仓库不会被删除。",
    deleteWorkspaceNameLabel: "输入 {{name}} 确认",
    deleteWorkspaceConfirm: "删除工作区",
    deletingWorkspace: "删除中…",
    addRepoTitle: "添加仓库",
    addRepoDesc: "引用本地仓库、克隆一个地址，或新建一个。",
    repoName: "名称",
    repoPath: "本地 git 路径",
    repoUrl: "Git 地址（可多个）",
    repoUrlPlaceholder: "https://github.com/acme/web-app.git\ngit@github.com:acme/api.git",
    repoLocation: "位置",
    repoMode_local: "本地",
    repoMode_clone: "克隆",
    repoMode_new: "新建",
    repoModeHint_local: "输入路径，或一次选择多个文件夹批量添加",
    repoModeHint_clone: "可一次粘贴多个 Git 地址批量导入，换行、空格或逗号分隔",
    repoModeHint_new: "在所选位置初始化一个新的空 git 仓库",
    addRepo: "添加仓库",
    cloneRepo: "克隆",
    createRepoCta: "创建",
    cloning: "克隆中…",
    cloneHint: "使用你的 git 凭据克隆到 {{path}}。",
    cloneRecognized: "将克隆 {{count}} 个仓库",
    cloneReposCta: "克隆 {{count}} 个仓库",
    importedToast: "已导入 {{count}} 个仓库",
    importFailed: "克隆失败",
    importSummary: "成功 {{ok}} 个 · 失败 {{failed}} 个",
    adding: "添加中…",
    addReposCta: "添加 {{count}} 个仓库",
    localRecognized: "待添加 {{count}} 个文件夹",
    addedToast: "已添加 {{count}} 个仓库",
    addFailed: "添加失败",
    addSummary: "成功 {{ok}} 个 · 失败 {{failed}} 个",
    removeRow: "移除",
    newHint: "在 {{path}} 创建，并带一个初始提交。",
    newThreadTitle: "新建 issue",
    newThreadDesc: "一个 issue 就是一条跨仓库推进的工作主线。",
    threadTitle: "标题",
    threadTitlePlaceholder: "issue 名称",
    threadType: "类型",
    threadTypePlaceholder: "选择类型",
    createThread: "创建 issue",
    creating: "创建中…",
    renaming: "重命名中…",
    taskName: "名称",
  },
  imMessages: {
    truncatedMarker: "……（内容已截断）",
    permissionTitle: "权限请求",
    permissionReplyCommand: "复制一条命令回复：",
    verdictAllowed: "已允许 ✓",
    verdictAlwaysAllowed: "已允许（总是）✓",
    verdictFullAccess: "已放行（任务全权）✓",
    verdictDenied: "已拒绝 ✕",
    verdictExpired: "已过期（回落工具自答）",
    verdictResolved: "已处理",
    humanQuestionTitle: "agent 提问",
    humanAnswerInstruction: "把末尾的 <回答> 替换成你的答复：",
    humanAnswerPlaceholder: "<回答>",
    humanAnswered: "提问已回答 ✓",
    answerPrefix: "答：",
    humanCancelled: "提问已取消：对应 workspace 已删除。",
    issueNotFound: "没有找到这个 issue。",
    bindThreadPrefix: "已绑定钉钉 thread 到",
    permissionAlreadyHandled: "这条权限请求已被处理或已过期。",
    humanAlreadyAnswered: "这个提问已被回答过了。",
    permissionCommandUsage:
      "格式不对。请发送：/allow <ask-id>、/deny <ask-id>、/always <ask-id> 或 /full <ask-id>。",
    humanAnswerUsage: "格式不对。请发送：/answer <issue-id> <ask-id> <回答>",
    threadRequired:
      "钉钉支持 thread，但机器人 API 不能可靠创建并返回新 thread ID。请先在话题圈创建或打开一个话题，再在该 thread 内发送 /bind <issue-id>；如果已经在 thread 内，请直接使用 /bind。",
    freeTextUnavailable:
      "自由对话（当前 IM 会话助理）需要桌面 app 运行上下文；当前路径无法处理，请回复卡片消息作答权限与提问。",
    unboundThread: "这个钉钉 thread 还没有绑定 Weft issue。请在当前 thread 发送 /bind <issue-id>。",
    conciergeDmPrefix: "钉钉私聊",
    conciergeGroupPrefix: "钉钉群聊",
    leadPrefix: "Lead：",
    resyncOne: "Weft 桥已上线，当前 1 项待你处理：",
    resyncMany: "Weft 桥已上线，当前 {n} 项待你处理：",
    resyncMore: " 等（+{n} 条）",
    resyncHint: "\n请使用每条钉钉提示中附带的命令作答；其他事项请在桌面端处理。",
  },
  settings: {
    title: "设置",
    subtitle: "默认项、语言和自动化。",
    backToApp: "返回应用",
    searchPlaceholder: "搜索设置…",
    groupPersonal: "个人",
    general: "通用",
    defaults: "默认项",
    generalHint: "这些设置影响新会话的默认体验；具体 issue 仍可覆盖。",
    appearance: "外观",
    interface: "界面",
    theme: "主题",
    system: "跟随系统",
    light: "亮色",
    dark: "暗色",
    language: "语言",
    languageHint: "界面文案语言。Agent 产出默认跟随它。",
    langZh: "中文",
    langEn: "English",
    projectsDirPlaceholder: "/Users/you/code",
    reviewSkill: "全局 review skill",
    reviewSkillPlaceholder: "superpowers:requesting-code-review",
    reviewSkillHint: "留空则自动检测（优先 superpowers 的 requesting-code-review）；review 在任务自己的会话里运行",
    autoReview: "自动 review",
    autoReviewHint: "任务流转到 review 列时，自动运行 review skill 并修复发现的问题",
    groupIntegrations: "集成",
    skills: "Skills",
    skillsHint: "以 git 托管的 skill 来源，同步后注入每个 agent 会话。",
    skillsFormats: "任意 git 地址(HTTPS、SSH 或本地路径)。skill 取自第一层 skills/ 目录,或 .claude-plugin/marketplace.json。",
    skillsAddSource: "添加来源",
    skillsGitUrlPlaceholder: "https://github.com/org/skills.git",
    skillsRefPlaceholder: "分支 / tag（可选）",
    skillsSync: "同步",
    skillsSyncing: "同步中…",
    skillsNever: "尚未同步",
    skillsNoSkills: "该来源未发现 skill。",
    skillsGlobal: "所有 workspace",
    skillsThisWs: "本 workspace",
    skillsGlobalOn: "已全局启用",
    skillsOverridden: "已被 {{source}} 覆盖",
    skillsRemove: "删除来源",
    notifications: "系统通知",
    notificationsHint: "窗口不在前台时，按下方勾选的事件弹出系统通知；点击通知可跳转到对应位置。Dock 角标始终反映待你处理数量。",
    notifyDenied: "系统已拒绝通知 — 打开系统设置开启",
    notifyDeniedUnavailable: "系统已阻止通知。",
    notifyCat_needs: "待你处理",
    notifyCat_review: "可以 review 了",
    notifyCat_quota: "进程额度已满",
    notifyQuietHours: "免打扰时段",
    notifyQuietHoursHint: "此时段内不弹系统通知；Dock 角标仍会更新。",
    notifyQuietStart: "免打扰开始",
    notifyQuietEnd: "免打扰结束",
    agentLanguage: "Agent 产出语言",
    agentLanguageHint: "plan / brief / commit / PR 文案的语言；代码与标识符始终英文。",
    diagnostics: "Agent 诊断",
    refreshDiagnostics: "刷新",
    diag_ok: "可用",
    diag_warning: "建议升级",
    diag_error: "不可用",
    agentLanguageZh: "跟随界面 · 中文",
    agentLanguageEn: "Follow UI · English",
    projectsDir: "项目目录",
    projectsDirHint: "新建和克隆的仓库存放在此。留空则每次询问。",
    choose: "选择文件夹",
    automation: "自动化",
    rules: "规则",
    automationEyebrow: "自动化",
    automationHint: "全自动的前提是异常循环有边界；常规流程不插入 Weft 自己的审批关。",
    defaultTool: "默认编程工具",
    defaultToolHint:
      "只影响新建的 issue 和新建的任务（批准写入时卡片上仍可逐条覆盖）。不会追溯改动已经在跑的 issue 的 lead 或任务的 worker——那些去各自的会话信息面板里切换。",
    noTools: "未检测到编程 CLI，请先安装 codex / claude / opencode / omp。",
    toolFallback: "{{configured}} 未安装，当前生效 {{tool}}。",
    agentCommands: "Agent 命令",
    agentCommandsHint:
      "若某个 CLI 以别名安装（例如 claude 被 alias 成 cc-claude），可在此设置 Weft 实际运行的命令。留空则使用默认名称，按 PATH 解析。",
    applyToExisting: "应用到已有会话",
    applyToExistingHint:
      "开启：已有 issue 与任务也切换到新命令。关闭：仅之后新建的会话生效，已有会话保留各自当前命令。",
    save: "保存",
    conservatism: "自动化尺度",
    conservatismHint: "控制 lead 自动拆解、派发和重试时的保守程度。",
    conservatismLow: "积极",
    conservatismMid: "均衡",
    conservatismHigh: "保守",
    issueBudget: "Issue 预算",
    issueBudgetHint: "用时间边界约束单条 issue 的自动推进。",
    issueBudgetValue: "空闲 {{idle}} 分钟 · 总时长 {{wall}} 分钟",
    reviewGroup: "Review",
    keepAwakeTitle: "运行时防止休眠",
    keepAwakeHint: "有会话运行时阻止系统进入空闲休眠，屏幕仍可正常熄灭。合盖或电池策略仍可能休眠。",
    remoteStandby: "远程待命",
    remoteStandbyHint: "桥启用期间保持系统唤醒，IM 指令随时可达。建议插电使用；合盖仍会休眠。",
    quotaFailoverGroup: "额度 fail-over",
    quotaFailoverTitle: "达限自动切换引擎",
    quotaFailoverHint:
      "某引擎报告额度达限时，自动把受影响的 issue/task 切到另一引擎（claude ⇄ codex）续跑。默认关闭——切换会把精简后的历史 digest 发给另一家厂商，这是只有你才能决定的成本与隐私取舍。各引擎当前用量见 Resources 面板。",
    quotaFailoverDisclosure:
      "开启后，结构化额度达限的 turn 可能把最近的对话上下文从一家厂商发给另一家；健康运行中的 turn 不会被中断。",
    quotaFailoverConfirm:
      "要开启额度自动 fail-over 吗？额度达限后，最近的对话上下文可能会从一家厂商发送给另一家。",
    autoMergeGroup: "自动合并",
    autoMergeTitle: "达标后自动合并",
    autoMergeHint:
      "当被跟踪的 PR/MR 达到本仓库「truly mergeable」的标准（CI 真正跑绿——不是没配置——、GitHub 自己的 review decision 通过、所有 review 讨论线程都已解决、无冲突），且经过一次新鲜复查确认这一状态仍然成立，Weft 会自动 squash 合并——不会再逐条询问你。默认关闭：这是一个不可撤销的动作（合并代码），且没有人确认这一次具体的合并。开启后，第一次检查就可能立刻、静默合并所有已经达标的被跟踪 PR/MR，不只是之后才达标的那些。每次尝试无论成功失败，都会在 PR 的时间线留下标记。",
    autoMergeDisclosure:
      "只对 Weft 已经在跟踪（通过 register_pr）、且宿主支持合并的 PR/MR 生效（目前仅 GitHub）。状态过期或探测正在失败的行会被跳过，不会用旧快照去合并。review 线程会被完整翻页读取，只要还有一条未解决就不会合并；如果线程数没能完整读到，一律按「未解决」处理，绝不当成已全部解决。仍然不检查的是某个仓库专属的机器人批准约定（例如某个 review 机器人的 👍）——这因仓库而异，也没有跨宿主的中立信号可读。如果你的仓库依赖它，请开启 GitHub 自己的分支保护（设置必需的 reviewer），这样条件不满足时合并会在服务端被直接拒绝。",
    autoMergeConfirm:
      "要开启自动合并吗？Weft 会自动 squash 合并每一个已经达到、或之后达到本仓库「truly mergeable」标准的被跟踪 PR/MR——不会再逐条问你，可能一开启就立刻合并好几个。它会检查 CI、GitHub 自己的 review decision、未解决的 review 线程和冲突，但不会判断某个 review 机器人是否已经放行。这个动作一旦发生就无法撤销。",
    engineRoutingGroup: "引擎路由",
    automaticRoutingTitle: "允许 Weft 自动选择引擎",
    automaticRoutingHint:
      "全局开关，作用于新 lead、任务、curator 对话和只读分析。普通工作优先 Codex，深度工作优先 Claude；手动选择始终保持 pin。",
    computerUseGroup: "Computer use",
    computerUseTitle: "允许 computer use",
    computerUseHint:
      "允许 agent 查看并操控本机其他应用窗口：枚举窗口、对指定应用窗口截图，并在你批准后向其注入点击、滚动、拖拽、文本输入与键盘快捷键。默认关闭。截图会排除 Weft 自身窗口与已知的终端应用窗口(尽力而为的清单，并非穷尽)。每次调用会先在「待你处理」中征求批准，但你在卡片上给出的 Always／Full 授权会自动放行后续同类调用；开启或关闭立即生效，对已在运行的会话也一样——下一次工具调用就会按当前设置校验，无需重启。",
    // 由 macOS（而非 webview）渲染：Automation（Apple Events）授权弹窗的用途
    // 说明。目录仍是唯一权威（CLAUDE.md：用户可见文案只经 en.ts/zh.ts）；
    // 生成器把这个 key 镜像进打包携带的本地化 InfoPlist.strings——修改后运行
    // `node --experimental-strip-types scripts/gen-infoplist-strings.mjs`，
    // 漂移会被 tests/frontend/infoPlistStrings.test.ts 直接判失败。
    computerUseMacosAutomationPrompt:
      "Weft 会在点击或输入前，通过 System Events 把你批准的 computer use 操作所指向的窗口调到最前。仅当你在设置中启用了 computer use、且批准了对应操作时才会执行。",
    computerControlActive:
      "有 agent 正在控制桌面（任务 {{dir}}，线程 {{thread}}）。按 Esc 或点击「立即停止」即可中断。",
    computerControlStop: "立即停止",
    computerControlStopFailed:
      "本次运行的急停已生效，但设置未能保存——重启后 computer use 可能重新启用。请重试，或检查磁盘空间是否充足。",
    detecting: "检测中…",
    installed: "已安装",
    installedVersion: "v{{version}}",
    updateRecommended: "建议升级",
    notInstalled: "未找到",
    dangerTitle: "危险模式",
    dangerDesc: "agent 运行所有工具（改文件、执行命令）都不再询问。快，但它们能在各自的 worktree 里做任何事。",
    irreversibleTitle: "不可逆边界",
    irreversibleHint: "合并受保护分支、生产部署和高成本动作必须停下来让你拍板。",
    mergeGate: "合并受保护分支",
    deployGate: "生产部署",
    moneyGate: "高成本动作",
    im: "IM",
    imProvider: "IM 桥",
    imProviderFeishu: "飞书 / Lark",
    imProviderDingtalk: "钉钉",
    imProviderSwitchHint: "一次连接一个渠道；切换不会删除另一渠道已保存的凭证。",
    imCollapsedHint: "已关闭——已保存凭证仍保留在本机。",
    imOnline: "在线",
    imConnecting: "连接中",
    imError: "异常",
    imOffline: "离线",
    imOpenPlatform: "飞书开放平台",
    imDingTalkOpenPlatform: "钉钉开放平台",
    imScanConnect: "扫码接入",
    imScanTitle: "扫码接入飞书",
    imScanHint: "用手机飞书「扫一扫」,授权创建一个 PersonalAgent 机器人即可。",
    imScanLoading: "正在生成二维码…",
    imScanSuccess: "已连接",
    imScanExpired: "二维码已过期。",
    imScanError: "接入失败,请重试。",
    imScanLarkUnsupported: "请使用飞书国内版账号(暂不支持 Lark 国际版)。",
    imScanRetry: "重新扫码",
    imModeScan: "扫码创建",
    imModeManual: "绑定已有",
    imScanModeHint: "用手机飞书扫码,创建一个新的 PersonalAgent 机器人,无需进开放平台。",
    imManualModeHint: "已有飞书应用?填入它的 App ID / App Secret 直接绑定。",
    imDingTalkManualModeHint: "绑定一个已发布、已启用机器人能力的钉钉企业内部应用。",
    imAppId: "App ID",
    imAppIdPlaceholder: "cli_xxxxxxxxxxxx",
    imAppIdHint: "在飞书开放平台创建自建应用；事件订阅选长连接并订阅 im.message.receive_v1。",
    imAppSecret: "App Secret",
    imAppSecretHint: "仅存本机 weft.db。留空表示保持已保存的值。启用且双凭证齐全后桥会连接。",
    imDingTalkClientId: "Client ID (AppKey)",
    imDingTalkClientIdPlaceholder: "dingxxxxxxxxxxxxxxxx",
    imDingTalkClientIdHint: "取自钉钉企业内部应用的凭证与基础信息。",
    imDingTalkClientSecret: "Client Secret (AppSecret)",
    imDingTalkClientSecretHint: "仅存本机 weft.db；留空保持已保存值。",
    imDingTalkSetupTitle: "开放平台配置",
    imDingTalkSetupStep1: "给企业内部应用添加“机器人”能力。",
    imDingTalkSetupStep2: "消息接收模式选择 Stream 模式，然后发布应用。",
    imDingTalkSetupStep3: "把机器人加入话题圈群；先私聊一句绑定 owner，再在目标 thread 内 @机器人执行 /bind。",
    imPermsLabel: "所需权限",
    imPermsHint: "",
    imPermsCopy: "复制权限点",
    imPermsCopied: "已复制",
    imBound: "已绑定 owner。",
    imUnbound: "未绑定——给 bot 发任意一句话即可绑定你的账号",
    imDingTalkOwnerTitle: "已绑定的钉钉 owner",
    imDingTalkOwnerResetHint:
      "更换账号前先重置当前 owner。重置后，第一个私聊机器人的钉钉账号会成为新 owner。",
    imDingTalkOwnerReset: "重置 owner",
    imDingTalkOwnerResetConfirm:
      "要重置已绑定的钉钉 owner 吗？新账号私聊机器人完成绑定前，第一个发起私聊的钉钉账号都可能成为新 owner。",
    imConnect: "保存并连接",
    imReconnect: "保存并重连",
    imRoutesGroup: "IM 会话绑定",
    imRoutesLabel: "已绑定的 issue",
    imRoutesHint: "每个 issue 1:1 映射到一条飞书话题。在话题里发送 /bind <issue-id> 绑定；话题消息会进 lead，lead 的回复会贴回话题。",
    imRoutesHintDingtalk: "钉钉话题圈使用真实 openConvThreadId。先创建或打开 thread，再在其中 @机器人发送 /bind <issue-id>；入站 thread 消息的回复通过临时 session webhook 返回原 thread。没有入站消息上下文时，不会把主动输出错发到父群。",
    imRoutesEmpty: "暂无绑定。",
    imRoutesLoading: "加载中…",
    imRoutesUnbind: "解绑",
    backup: "备份",
    backupTitle: "备份",
    backupGroupRemote: "Git 远端",
    backupGroupSchedule: "定时",
    backupGroupRecovery: "Recovery Key",
    backupGroupRestore: "恢复",
    backupGroupStatus: "状态",
    backupEnable: "启用备份",
    backupEnableHint:
      "把加密后的 weft.db 快照 push 到私有 git 仓。Recovery Key 不上传远端。",
    backupRemoteLabel: "Git 仓库 URL",
    backupRemoteHint: "推荐 SSH，或 file:// 路径。指向一个空的私有仓；Weft 会把快照 push 上去。",
    backupRemotePlaceholder: "git@github.com:me/weft-backup.git",
    backupTest: "测试连接",
    backupTesting: "测试中…",
    backupTestOk: "已连通",
    backupAuto: "自动备份（每小时）",
    backupAutoHint: "Weft 打开期间，按间隔自动跑一次快照。",
    backupOnExit: "退出时备份",
    backupOnExitHint: "退出前再跑一次快照，限时 10 秒。",
    backupRunNow: "立即备份",
    backupRunning: "备份中…",
    backupSave: "保存",
    backupSaving: "保存中…",
    backupExportRecoveryKey: "下载 Recovery Key",
    backupRecoveryKeyHint: "没有这个文件就无法解密备份。请离线或用密码管理器保管。",
    backupRecoveryKeyDialog:
      "备份不包含解密密钥。请下载 Recovery Key 文件并妥善保管（密码管理器或离线介质）。没有这个文件，备份将无法恢复。",
    backupRecoveryKeyDownload: "下载 Recovery Key",
    backupRecoveryKeyPlaceholder: "/path/to/weft-recovery-key.json",
    backupCancel: "取消",
    backupStatusNever: "从未备份",
    backupStatusAgo: "上次备份",
    backupNextAt: "下次备份",
    backupRestoreLink: "从备份恢复",
    backupRestoreHint: "用于全新机器。若 Weft 主目录已有 weft.db，会拒绝执行。",
    backupRestoreRemoteLabel: "要恢复的远端 URL",
    backupRestoreKeyLabel: "Recovery Key 文件",
    backupRestorePickKey: "选择 Recovery Key…",
    backupRestoreConfirm: "恢复",
    backupRestoring: "恢复中…",
    backupRestoreSuccess: "已恢复。请重启 Weft。",
    backupRestoreNoKey: "请先选择 Recovery Key 文件。",
    security: "安全",
    encryption: "数据库加密",
    encryptionHint: "使用密码保护你的数据库。如果丢失密码，数据将无法恢复。",
    encryptionEnabled: "已加密",
    encryptionDisabled: "未加密",
    enableEncryption: "启用加密",
    disableEncryption: "关闭加密",
    changePassword: "修改密码",
    encryptionNewPassword: "新密码",
    encryptionConfirmPassword: "确认密码",
    encryptionCurrentPassword: "当前密码",
    encryptionPasswordMismatch: "两次输入的密码不一致",
    encryptionPasswordEmpty: "密码不能为空",
    encryptionEnableWarning: "将对你的数据库进行加密。请妥善保管密码——如果丢失，数据将无法恢复。",
    encryptionDisableWarning: "将移除数据库加密。你的数据将以明文存储。",
    encryptionRestartRequired: "加密设置已更改。正在重启 Weft…",
    encryptionSuccess: "加密设置已应用。需要重启。",
    encryptionError: "加密操作失败：{{error}}",
    groupSystem: "系统",
    resources: "运行时资源",
    resourcesHint:
      "只读展示这台机器当前的进程压力：既有全系统的进程配额空间，也有 Weft 自身进程树、内存和并发会话槽位的占用。采样本身由进程树安全网完成，这个页面只负责展示。",
    resourcesStale: "刷新失败，当前展示的是最近一次读到的数据。",
    resourcesLoadFailed: "刷新失败，目前还没有取到任何数据。",
    resourcesQuotaGroup: "进程配额",
    resourcesQuotaNormal: "正常",
    resourcesQuotaWarning: "接近上限",
    resourcesQuotaDegraded: "已降级",
    resourcesQuotaNoLimit: "当前平台未报告进程数上限。",
    resourcesEngineQuotaGroup: "引擎额度",
    resourcesEngineQuotaOk: "正常",
    resourcesEngineQuotaWarning: "接近上限",
    resourcesEngineQuotaExceeded: "已达限",
    resourcesEngineQuotaEmpty: "本次运行还没有观测到额度信号——发一轮消息后再来看。",
    resourcesEngineQuotaPercent: "已用 {{percent}}%",
    resourcesEngineQuotaResetsDays: "{{days}} 天 {{hours}} 小时后重置",
    resourcesEngineQuotaResetsHours: "{{hours}} 小时 {{minutes}} 分钟后重置",
    resourcesEngineQuotaResetsMinutes: "{{minutes}} 分钟后重置",
    resourcesTreeGroup: "进程树",
    resourcesTreeTotal_other: "共 {{count}} 个进程",
    resourcesTreeEmpty: "当前没有 Weft 派生的进程在运行。",
    resourcesOwnerGlobalAppServer: "全局 codex",
    resourcesOwnerSession: "Worker 会话",
    resourcesOwnerLeadThread: "Lead 会话",
    resourcesOwnerCurator: "仓库分析",
    resourcesOwnerOpencode: "opencode",
    resourcesOwnerPreview: "预览服务",
    resourcesOwnerProbe: "探测",
    resourcesOwnerOther: "其它",
    resourcesMemoryGroup: "内存",
    resourcesMemoryUnavailable: "当前平台不支持采样",
    resourcesMemoryHint:
      "Weft 自身进程树（本体 + 它派生的全部 agent / MCP 进程）占用的常驻内存，不是整机内存。",
    resourcesSessionsGroup: "并发会话",
    resourcesSessionsLabel: "{{active}} / {{max}} 个活跃槽位",
    resourcesSessionsFull: "已满",
    resourcesSessionsHint: "超过上限的新会话会排队等待，不会被拒绝。",
  },
  danger: {
    nudgeTitle: "经常在批准?",
    nudgeBody: "开启危险模式，跳过所有授权提示。",
    notNow: "以后再说",
    turnOn: "开启",
    enabled: "危险模式已开启——agent 不再询问。",
    undo: "撤销",
  },
  error: {
    title: "这个界面崩溃了",
    body: "其余界面与后台 agent 不受影响。",
    retry: "重试",
    stackTrace: "堆栈信息",
    renameDuplicate: "已有同名条目。",
    renameEmpty: "名称不能为空。",
    renameFailed: "重命名失败，请重试。",
    deleteWorkspaceFailed: "删除工作区失败，请重试。",
    deleteWorktreeFailed: "删除 worktree 失败，请重试。",
  },
  status: {
    running: "运行中",
    idle: "空闲",
    exited: "已退出",
  },
  readiness: {
    status: {
      review_ready: "可进入评审",
      blocked: "已阻塞",
      needs_you: "需要你处理",
      unknown: "证据未知",
      failed: "已失败",
      unavailable: "就绪状态不可用",
    },
    reason: {
      no_active_lanes: "没有活跃 Lane",
      upstream_unmet: "上游交付尚未完成",
      evidence_missing: "就绪证据仍在加载",
      remote_unknown: "远端交付证据不可用",
      execution_drifted: "工作树分支与 Lane 不一致",
      policy_gate_pending: "等待策略闸门确认",
      open_need: "需要人工回答",
      checks_failing: "验证检查失败",
      checks_unknown: "尚未产出验证检查",
      worker_failed: "Worker 以错误结束",
      in_progress: "交付仍在进行中",
      pr_ci_pending: "PR CI 仍在等待",
      pr_ci_failing: "PR CI 失败",
      pr_review_changes_requested: "PR 评审要求修改",
      pr_threads_unresolved: "PR 评审线程未解决",
      pr_conflict: "PR 存在合并冲突",
      pr_closed_unmerged: "PR 已关闭但未合并",
    },
  },
  changeSet: {
    tab: "变更集",
    title: "变更集",
    loading: "变更集加载中…",
    loadFailed: "变更集加载失败。",
    empty: "还没有任务——这个 issue 目前不会写入任何仓库。",
    laneCount_other: "{{count}} 条活跃任务",
    wave: "第 {{index}} 步",
    waveWaits: "需要等上一步完成",
    whyRepo: "为什么选这个仓库",
    openLane: "打开该任务的会话",
    branchUnset: "尚未分配分支",
    baseDefault: "仓库默认分支",
    dependsOn_other: "依赖 {{count}} 条任务",
    evidenceCounts: "证据 {{fresh}} 最新 · {{stale}} 已过期 · {{unknown}} 未知",
    evidenceNone: "暂无证据记录",
    checkout: {
      notProbed: "这次读取没有采样工作区。",
      noneRegistered: "还没有登记工作区。",
      unsampled: "工作区无法读取",
      dirty: "有未提交改动",
      differsFromDeclared: "与声明分支不一致",
    },
    status: {
      queued: "排队中",
      planning: "规划中",
      working: "构建中",
      review: "评审中",
      done: "已完成",
      unknown: "状态未知",
      not_materialized: "尚未开始",
    },
    evidenceUnscanned: "证据未完整扫描",
    cycle: "循环依赖",
    cycleBody: "这些任务互相等待——没有任何顺序能满足",
    pr: {
      unidentified: "PR（记录缺失）",
      probeFailed: "host 探测失败",
      stateUnknown: "状态未知",
      ciUnknown: "CI 未知",
      ciNotConfigured: "无 CI",
      ciPending: "CI 进行中",
      ciPassing: "CI 通过",
      ciFailing: "CI 失败",
      reviewUnknown: "评审状态未知",
      reviewChangesRequested: "要求修改",
      reviewAwaiting: "等待批准",
      reviewApproved: "已批准",
      threadsUnchecked: "未检查讨论串",
      threadsUnknown: "讨论串状态未知",
      threadsResolved: "讨论串已解决",
      threadsUnresolved: "讨论串未解决",
      conflictUnknown: "可合并性未知",
      conflictClean: "无冲突",
      conflicting: "存在冲突",
    },
    reconciliation: {
      matched: "工作区一致",
      drifted: "工作区已漂移",
      unknown: "工作区状态未知",
    },
    checks: {
      not_applicable: "尚未到验证阶段",
      not_produced: "没有验证结果",
      passed: "验证通过",
      failing: "验证失败",
    },
    upstream: {
      satisfied: "上游已就绪",
      unmet: "上游尚未交付",
      unknown: "上游状态未知",
    },
  },
  evidence: {
    title: "证据",
    empty: "暂无证据记录。",
    loading: "证据加载中…",
    loadFailed: "证据加载失败。",
    supersededBadge: "已被取代",
    freshness: {
      fresh: "最新",
      stale: "已过期",
      unknown: "未知",
    },
    kind: {
      code: "代码",
      verification: "验证",
      interface: "接口",
      host: "Host",
      execution: "执行对账",
      decision: "决策",
      handoff: "交接",
    },
  },
  grants: {
    inherited: "已继承授权",
    inheritedTitleFull: "这个 issue 的完全访问授权从上次运行保留了下来——点击撤销",
    inheritedTitleAlways_other:
      "这个 issue 的 {{count}} 条「始终允许」规则从上次运行保留了下来——点击撤销",
    inheritedTitleBoth:
      "这个 issue 的完全访问授权和「始终允许」规则从上次运行保留了下来——点击撤销",
    revokeTitle: "撤销继承的授权？",
    revokeBodyFull:
      "该 issue 下各任务保留的完全访问授权会被清除。下次有工具需要权限时会重新问你。",
    revokeBodyAlways_other:
      "该 issue 下各任务保留的 {{count}} 条「始终允许」规则会被清除。下次有工具需要权限时会重新问你。",
    revokeBodyBoth:
      "该 issue 下各任务保留的完全访问授权和「始终允许」规则会被清除。下次有工具需要权限时会重新问你。",
    revokeConfirm: "撤销授权",
    revokeFailed: "撤销失败——该授权仍然有效，请重试。",
    grantNotSaved: "已临时授权，但未能保存——重启后可能会再次询问。",
    // issue #103:只读自动放行——与上面完全访问/始终允许是两套独立授权，只存内
    // 存、不落盘（重启即失效），且恒不覆盖写入或网络/凭证类请求。
    readOnlyIssue: "只读已放行",
    readOnlyIssueTitle: "批准派发时，已把只读请求的信任下放给该 issue 的每个 worker——点击撤销",
    revokeReadOnlyTitle: "撤销只读放行？",
    revokeReadOnlyIssueBody:
      "该 issue 下的每个 worker 都会停止自动放行只读请求——下一次会重新询问你。不影响完全访问或始终允许规则。",
    readOnlyReleased_other: "已放行 {{count}} 个只读请求。",
  },
  ai: {
    shellSnippet: "Shell 片段",
  },
  kind: {
    feature: "功能",
    bugfix: "缺陷",
    refactor: "重构",
    spike: "探索",
  },
  actionCard: {
    addRepoLabel: "导入已有目录",
    newRepoLabel: "新建仓库",
    cloneRepoLabel: "克隆仓库",
    readOnlyHint: "这张卡片属于较早的轮次。",
    resolved: "已添加 {{name}}",
  },
  planCard: {
    label: "方案",
    requirements: "需求与验收",
    approach: "技术方案",
    split: "拆分预览",
    risks: "未解决风险",
    approve: "同意，开拆",
    reviseHint: "要调整？直接在下方回复。",
    readOnlyHint: "这份方案属于较早的轮次。",
    approved: "方案已确认 · {{name}}",
    deliverFailed: "lead 没有收到确认——它可能已停止。重启 lead 后再次确认。",
    basedOnCases_other: "基于 {{count}} 条测试用例",
    viewCases: "查看",
  },
  testPlan: {
    title: "测试用例",
    defaultTitle: "测试用例",
    caseCount_one: "{{count}} 条用例",
    caseCount_other: "{{count}} 条用例",
    open: "查看与编辑",
    edit: "编辑",
    editHint: "双击改文本 · Tab / Enter 增节点 · 拖拽调层级",
    fullscreen: "全屏",
    save: "保存",
    cancel: "取消",
    loading: "脑图加载中…",
    empty: "还没有测试用例——可以让 lead 先推导，比如说「先出测试用例」。",
    emptyError: "测试用例不能保存为空。",
    savedLeadOffline: "已保存。lead 已停止，下次对话时会读到最新内容。",
    refreshedWhileEditing: "lead 更新了测试用例——为避免覆盖新版本，未保存的草稿已丢弃。",
    zoomIn: "放大",
    zoomOut: "缩小",
    fitView: "适配视图",
    editedNotice: "我在面板里更新了测试用例——继续之前请先读最新版本。",
    notePlaceholder: "针对这个节点提问或提修改建议…",
    ask: "提问",
    suggest: "建议修改",
    askPrefix: "提问：",
    suggestPrefix: "修改建议：",
  },
  settled: {
    permissionAllow: "已批准 · {{summary}}",
    permissionDeny: "已拒绝 · {{summary}}",
    permissionAlways: "已始终允许 · {{summary}}",
    permissionFull: "已授予完全访问 · {{summary}}",
    askAnswered: "已回答 · {{text}} → {{answer}}",
  },
  repoActions: {
    chooseDir: "选择目录",
    chooseParentDir: "选择父目录",
    repoNameTitle: "仓库名",
    repoNamePlaceholder: "my-project",
    repoUrlTitle: "Git URL",
    repoUrlPlaceholder: "https://github.com/...",
    addedToast: "已添加仓库 {{name}}",
    failedToast: "失败：{{message}}",
    staleCard: "这条仓库操作已失效",
    noWorkspaceToast: "无法准备工作区",
    completedWithoutRepository: "仓库操作已完成，但没有返回仓库信息。",
  },
  slashLocal: {
    testCases: "推导测试用例",
    testCasesPrompt: "请先为这个 issue 推导测试用例（思维导图形式），再讨论技术方案。",
    addRepo: "导入已有目录",
    newRepo: "新建仓库",
    cloneRepo: "克隆仓库",
  },
  emptyMap: {
    addRepoBtn: "导入",
    newRepoBtn: "新建",
    cloneRepoBtn: "克隆",
  },
  updater: {
    newVersion: "Weft {{version}} 已发布",
    install: "更新并重启",
    dismiss: "忽略",
  },
} as const;
