...
pub async fn confirm(
    ...
    if let Some(want) = &expected_version {
        if &start_plan.created_at != want {
            // Version mismatch: the shown proposal was superseded. Return None (NOT an empty
            // Vec) so the frontend can distinguish this from a legitimate confirm that
            // dispatched zero lanes — only None routes to Review/refresh.
            return Ok(None);
        }
    }
...
mod tests {
    ...
    async fn confirm_stale_version_refuses_matching_dispatches() {
        ...
        assert!(refused.is_none(), "a version mismatch returns None (distinct from a zero-lane confirm)");
        ...
        assert_eq!(ids.expect("confirmed (not a version mismatch)").len(), 1, "the matching version dispatches the one direction");
    ...
    async fn confirmed_plan_survives_late_withdraw() {
        ...
        save_proposal(&db, t.id, &proposal).await.unwrap();
        let confirmed_ids = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
    ...
    async fn confirm_is_idempotent_no_duplicate_directions() {
        ...
        save_proposal(&db, t.id, &proposal).await.unwrap();
        let first = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
        assert_eq!(first.len(), 1);
        assert_eq!(repo::list_directions(&db, t.id).await.unwrap().len(), 1);
        // A second confirm (e.g. after a partial-failure retry) must NOT duplicate the
        // already-created lane.
        let _second = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
    ...
    async fn confirm_retry_returns_existing_id_for_dispatch() {
        ...
        let second = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
    ...
    async fn confirm_is_atomic_rolls_back_all_on_failure() {
        ...
        let ids = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
    ...
    async fn thread_gate_serializes_concurrent_confirms() {
        ...
        let ids1 = r1.expect("first confirm ok").expect("confirmed (not a version mismatch)");
        let ids2 = r2.expect("second confirm ok").expect("confirmed (not a version mismatch)");
    ...
    async fn confirm_reuse_rematerializes_reclaimed_worktree() {
        ...
        save_proposal(&db, t.id, &prop).await.unwrap();
        let first_ids = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
        ...
        save_proposal(&db, t.id, &prop).await.unwrap();
        let second_ids = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
    ...
    async fn confirmed_fast_path_excludes_individually_approved_lanes() {
        ...
        let confirm_ids = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
        ...
        let retry_ids = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
    ...
    async fn confirmed_fast_path_redispatches_by_recorded_id() {
        ...
        let retry = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
    ...
    async fn confirmed_fast_path_legacy_single_candidate_requires_base_compat() {
        ...
        let retry2 = confirm(&db, t.id, None).await.unwrap().expect("confirmed (not a version mismatch)");
