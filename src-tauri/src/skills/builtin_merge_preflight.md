...
- Treat validation as code execution. For an external or otherwise untrusted candidate, do not run its validation commands in the normal agent environment. Require explicit human trust confirmation or use a disposable, credential-free isolated sandbox/container with network disabled, a fresh non-user home, and no host credential or shared-Git mounts; otherwise report that validation was withheld and limit the preflight to non-executing inspection.

## Protocol

1. Resolve the refs before changing anything. Identify the target/base repository and ref, then separately identify the candidate head repository and ref or provider-specific PR ref. Fetch the latest base from the target remote. Fetch the candidate from its head remote or provider-specific PR ref, not by assuming its branch exists on the target remote. When a fork head is not already available locally, use a direct-URL fetch inside a disposable isolated clone; never run `git remote add` or write persistent cross-repository wiring into the canonical repository. Resolve and record the post-fetch base SHA and candidate SHA, and for a pull request verify the fetched candidate SHA exactly matches the hosting provider's advertised head SHA. Do not use stale pre-fetch SHAs.

2. Create a unique temporary directory and rehearsal checkout. When the base and candidate objects are already local, add a detached worktree at the latest base SHA with `git worktree add --detach <temporary-worktree> <base-sha>`. When an isolated clone was needed for a fork head, use that clone as the disposable rehearsal checkout instead of sharing the canonical repository's Git directory. Do not use the original working tree as the merge workspace.
...
4. Only after a clean merge, decide whether validation may execute. For an external or otherwise untrusted candidate, an ordinary temporary worktree is not a sandbox: it can still access credentials, the home directory, network, and shared Git state. Obtain explicit human trust confirmation or use a disposable isolated sandbox/container with network disabled, a fresh non-user home, no host credential or shared-Git mounts, and only the material needed for the rehearsal before running candidate-controlled commands. Without one of those conditions, report the clean/conflicted merge result and that validation was not run.
