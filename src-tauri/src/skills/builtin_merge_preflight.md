...
- Never merge, reset, check out, or run validation in the original working tree. Use a uniquely named temporary rehearsal checkout: a detached worktree only for explicitly trusted local objects, or a standalone isolated clone for a fork or sandboxed candidate.
- Never push, force-push, update a remote ref, call a GitHub merge API, or perform a remote merge automatically.
- Register cleanup before creating any temporary checkout, isolated clone, bundle, or worktree. Cleanup must run on success, a conflict, validation failure, and interruption.
...
1. Resolve the refs before changing anything. Identify the target/base repository and ref, then separately identify the candidate head repository and ref or provider-specific PR ref. Fetch the latest base from the target remote. Fetch the candidate from its head remote or provider-specific PR ref, not by assuming its branch exists on the target remote. When a fork head is not already available locally, first allocate a unique clone path and register clone-specific cleanup, then use a direct-URL fetch inside a disposable `git clone --no-checkout`; never run `git remote add` or write persistent cross-repository wiring into the canonical repository. If that clone or a later fetch fails, clean up its registered path before continuing or reporting failure. Resolve and record the post-fetch base SHA and candidate SHA, and for a pull request verify the fetched candidate SHA exactly matches the hosting provider's advertised head SHA. Do not use stale pre-fetch SHAs.
...
2. Before checking out or merging the candidate, decide whether the candidate is external or otherwise untrusted. An ordinary temporary worktree is not a sandbox: configured Git filters or merge drivers can run during checkout or merge. Obtain explicit human trust confirmation or use the isolated environment described above before proceeding. Without one of those conditions, report the resolved refs and that checkout, merge, and validation were not run.
   ...
   Once authorized, create a unique temporary rehearsal checkout. With explicit human trust, when the base and candidate objects are already local, add a detached worktree at the latest base SHA with `git worktree add --detach <temporary-worktree> <base-sha>`. When an isolated clone was needed for a trusted fork head, use that clone as the disposable rehearsal checkout and explicitly run `git checkout --detach <base-sha>` after fetching before merging the candidate. When the isolated sandbox/container is the authorization, never use `git worktree add`: create a standalone disposable clone inside that environment, seeded only with the recorded immutable base and candidate objects, such as a read-only disposable Git bundle produced by non-executing ref/object operations. Register cleanup for the bundle and clone before creating either, then explicitly check out the recorded base SHA in the standalone clone. Do not mount or otherwise share the canonical repository's `.git` directory. Do not use the original working tree as the merge workspace.
...
6. Clean up on every exit path. Abort an in-progress merge in the rehearsal checkout when needed. For a linked temporary worktree, use `git worktree remove --force <temporary-worktree>` and then remove its unique temporary directory. For every isolated clone, delete the clone directory directly after aborting its merge; do not call `git worktree remove` on a clone root. Delete every registered disposable bundle after its sandboxed clone is gone. Verify every registered temporary path is gone. If interruption prevents complete cleanup, report the remaining path and finish cleanup before any later attempt. Never push, force-push, or perform a remote GitHub merge as part of this protocol.
