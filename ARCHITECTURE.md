# 本地多仓交付编排器 —— 架构设计与可行性评估

> 一句话定位:一个**本地优先、无服务端**的多仓交付编排器。用户给出一个需求，Weft 读取逻辑工作区的仓库地图，拆出有边界的 agent 工作通道，并 **headless 驱动用户自己的 Claude Code / Codex / OpenCode** 跨多个仓库推进，把需求从意图带向实现、PR、合并和上线；当前实现落地到可 review 的 worktree diff + pre-PR checks，后续边界继续向 PR / merge / release 推进。任何会话都可随时在用户自己的终端接管；团队共享通过**配置下发**(git / plugin marketplace)实现。
...
本仓库已经不是纯设计稿。当前代码落地的是最终目标链路的前半段：**需求 → 仓库地图 → 写入范围确认 → 仓库原生 worktree worker → 可 review 的 diff / pre-PR checks**。产品最终目标仍是 **需求 → PR → 合并 → 上线** 的本地多仓自动交付编排。
...
- **scope 口径**:读仓不再建模为 `read/none` 标签;agent 可自由读取 workspace repo。只有“要写哪个仓”需要声明和物化。Lead 通过 planner MCP `propose_directions` 产出 direction 列表,每项必须给一个写仓和理由;UI 把它作为待确认的写入声明放进 Needs-you,可单项 approve/deny,也可整体 confirm。
- **尊重仓库习惯**:worktree 创建在目标仓库自己的 `.worktrees/weft/<branch>` 下；分支名从目标 repo 既有本地/remote 分支推断风格(`feat/*` vs `feature/*`、`fix/*` vs `bugfix/*`)，冲突时追加数字后缀。Weft 的状态和路由留在本地 DB，不把内部 `ws/...` 命名强加给用户仓库。
...
- **配置注入与权限**:本地 axum server 同时承载 thread bus MCP、planner MCP、Ask Bridge。Claude 通过临时 `--mcp-config`/`--settings` 注入;Codex 通过稳定的全局 hook + worktree 内的路由文件接入 Ask Bridge,不使用 `--dangerously-bypass-hook-trust`;OpenCode 在一次性 worktree 写 `.opencode/plugins/weft-ask.js` 和 merge `opencode.json`。生成文件会尽量写入 worktree 的 git exclude,不写入目标仓库的受版本管理配置。
...
尚未落地为产品行为:自动创建 PR、合并受保护分支、staging/production 部署编排、团队 marketplace 同步、长期语义 Curator agent、完整 PR/CI/CD 状态观测。下文保留最终目标设计,但凡与本节冲突,以本节为当前代码事实。
...
7. **尊重仓库习惯:工作目录可在仓内,配置不进仓库历史**(见 2.1)。工作目录按目标 repo 习惯放在 `.worktrees/weft/<branch>`，但跨仓库访问关系与会话配置只通过临时参数/一次性 worktree 注入,不写进受版本管理的仓库配置,保证单仓独立、工作区互不污染、零累积。
8. **产品化:屏蔽机制,呈现决策与结果**(见 4.7)。隐藏 plumbing(worktree/headless 进程/MCP/add-dir/旁路),用产品词表达;但用户负责的"决策与结果"(scope、仓库原生分支/PR/合并/上线、工具选择)留在台前。隐藏三件套:抽象 + 逃生舱(真路径/开终端)+ 失败可读。
9. **多语言(中/英)从第一天就内建**(见 4.8)。UI 文案外置 + agent 产出语言两层都要,绝不硬编码字符串。
10. **Automation-first,Weft 不自加审批关**。产品最终目标是自动化:lead 默认自动分解→spawn→派发→驱动到实现、PR、合并和上线。**唯一的阻塞性人工来自工具自身的权限/审批习惯**(Codex/Claude 按用户自己的配置弹出),Weft 只透传(见 4.3),不新增 gate。人是监督/随时介入/异常处理,不是必经关卡。仅"不可逆/爆炸半径大"的动作(合并受保护分支、生产发布、破坏性/资金操作)留**可配置**边界,默认由用户定;git/CI/工具权限已是安全网。
11. **交付边界分阶段推进**。当前代码已经到仓库原生 worktree diff + pre-PR checks;下一产品边界是 Task → PR。merge / CI-CD / release 先接入仓里现有 harness，再由 Weft 逐步编排。因 Weft 驱动原生 CLI(不绕 hooks),worker 后续开/更新 PR 时**仓库自身的 PR 触发 hooks/CI 自然触发**——交付即自动接上现有 PR harness,接缝在 PR。CI/CD 反应式、更高维 harness 为未来维度(架构经 git/forge+bus 已可容纳),但最终目标是需求一路做到合并和上线。
...
│           └── branch                   # repo-style branch, e.g. feat/add-login or feature/add-login
└── distribution                        # 团队下发来源
      ├── marketplaceRef (git)          # plugin marketplace 仓
      └── version / pinned

Materialization (按 thread×方向 运行时生成)
├── <repo>/.worktrees/weft/<branch>/      # 仓库内 git worktree
│     ├── (agent 注入文件, 全部写入 git exclude)
│     └── (该 direction 的真实代码改动)
├── branch                               # 遵循目标仓库风格: feat/* vs feature/*, fix/* vs bugfix/*
└── 说明: 同一 .git 可派生多个 worktree → 跨 thread 并行时仍共享对象库、分支隔离；Weft 不再把内部 ws/<workspace>/<thread>/<direction> 命名写入用户 repo。
...
### 2.1 关键隔离原则:工作目录贴近仓库,配置不污染仓库历史

Weft 尊重目标仓库自己的协作习惯：工作目录可以放在该仓库下的 `.worktrees/weft/<branch>`，
分支名也按该仓库已有风格生成。但跨仓库访问关系和会话级配置有"临时"与"持久"两种形态，**编排器只用临时形态**，否则会把工作区配置写进仓库历史，让单仓使用变得不干净:

| 工具 | 临时形态(用这个) | 持久形态(别写进受版本管理的仓库配置) |
|---|---|---|
| Claude Code | `--add-dir <path>`(启动 flag)、`/add-dir`(会话内)——仅当前会话,不落盘 | `additionalDirectories` 写进仓的 `.claude/settings.json`(提交后永久跟随该仓) |
| Codex | `--add-dir` / `-C` / `-c key=val`(启动参数)——临时；Ask Bridge 走稳定的全局 hook + worktree 内的路由文件 | `writable_roots` 写进仓的 `config.toml`，或动态 per-session hook 写进用户根配置 |

**规则**:工作区的跨仓库访问关系与会话级配置,只活在 **(a) 临时启动参数 / `--settings` / env**,或 **(b) 仓库内 worktree 的 git exclude 文件**(随 worktree 用完即删)。**永远不进目标 repo 的受版本管理配置。**

由此得到三条保证:

1. **单独改一个仓**:直接打开原始 checkout,只加载该仓自身 + 用户配置,看不到任何工作区配置 → 干净。
2. **推进 workspace B**:B 创建自己的仓库内 worktree + 自己的启动参数,不继承 A 的任何配置。
3. **仓库历史零累积**:Weft 文件在 `.worktrees/weft` 或 git exclude 中,不会进入仓库历史。
...
- **真正的工程难点**:lead 在大仓上做跨仓 scope 分解时不能爆上下文 → 靠持久化的 **Repo Profile + 跨仓依赖图(见 4.9)** 当紧凑地图,只在需要时定向读码,而非 ingest 全部。这是"自动判断跨仓写入范围"这个核心亮点的成败点,建议早做原型。
...
**归宿**:主存 Weft 本地库(快、含跨仓图);**单仓一句话职责可选写回该仓 AGENTS.md**(可移植、裸 CLI 也吃得到);跨仓图始终在 Weft。

**正反馈**:用户纠正会持久化,plan 越用越准。Profile + 图 = lead 的紧凑"地图",让"跨仓 scope 自动分解"这个核心亮点在大仓上可行而不爆上下文。
...
4. **按 direction 懒物化执行**:
   - 仅对 approved/confirmed direction 的写仓按目标 repo 既有分支风格开分支，并在该 repo 的 `.worktrees/weft/<branch>` 下开 worktree。
   - 注入 thread bus MCP + Ask Bridge + worker brief → 起该方向指定工具的会话(可异构);weft 会话界面实时看,sidecar 聚合。
5. **当前收尾边界**:按 worktree 看 diff → 跑 pre-PR checks / review skill → 人 review。下一产品边界是自动开/更新 PR；最终目标继续覆盖合并与上线。
...
- **分支遵循目标 repo 风格**:从目标 repo 现有本地/remote 分支推断 `feat/*` vs `feature/*`、`fix/*` vs `bugfix/*` 等；无可判断样本时回退到 `feature/<slug>` / `fix/<slug>`。同名冲突追加 `-2`/`-3`，避免两个 worktree 检出同一分支。
- **同仓多 worktree**:一个仓被多个"thread×方向"同时写时各派生一个仓内 worktree(共享对象库,零拷贝),路径为 `<repo>/.worktrees/weft/<branch>`；产品做命名空间管理 + 控制磁盘/依赖成本(每 worktree 一份依赖 → 懒装/链接共享)。
...
### 5.3 首用流(onboarding,~5 分钟看到核心价值)

![5 分钟首用流](docs/diagrams/weft-first-use.svg)

目标:让新用户 ~5 分钟内体验到核心价值(自动判断跨仓写入范围)。步骤:① 新建 workspace + 加第一个仓 → ② Curator 自动 profile(确认/改一行)→ ③ 再加仓,依赖图自动成形 → ④ 新建 thread、输入一个 **Task**(PRD/bug/重构/spike)→ ⑤ **Lead 自动跨仓 scope 分解**(动哪几个仓 + 怎么分工)→ 之后 automation-first 自动拉起 worker、验证、出 PR。全程产品词、机制不可见。
...
- 更干净的 OpenCode 临时注入,避免修改被 Git 跟踪的 `opencode.json`。
