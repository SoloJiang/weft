...
> 一句话定位:一个**本地优先、无服务端**的多仓交付编排器。用户给出一个需求，Weft 读取逻辑工作区的 repo map，拆出有边界的 agent lanes，并 **headless 驱动用户自己的 Claude Code / Codex / OpenCode** 跨多个仓库推进，把需求从意图带向实现、PR、合并和上线；当前实现落地到 reviewable worktree diff + pre-PR checks，后续边界继续向 PR / merge / release 推进。任何会话都可随时在用户自己的终端接管；团队共享通过**配置下发**(git / plugin marketplace)实现。
...
本仓库已经不是纯设计稿。当前代码落地的是北极星链路的前半段：**Requirement → repo map → write-scope review → repo-native worktree worker → reviewable diff / pre-PR checks**。产品北极星仍是 **Requirement → PR → merge → release** 的本地多仓自动交付编排。
...
- **repo origin 口径**:worktree 创建在目标仓库自己的 `.worktrees/weft/<branch>` 下；分支名从目标 repo 既有本地/remote 分支推断风格(`feat/*` vs `feature/*`、`fix/*` vs `bugfix/*`)，冲突时追加数字后缀。Weft 的状态和路由留在本地 DB，不把内部 `ws/...` 命名强加给用户仓库。
...
- **注入与权限**:本地 axum server 同时承载 thread bus MCP、planner MCP、Ask Bridge。Claude 通过临时 `--mcp-config`/`--settings` 注入;Codex 通过 stable global hook + worktree-local route 文件接入 Ask Bridge,不使用 `--dangerously-bypass-hook-trust`;OpenCode 在一次性 worktree 写 `.opencode/plugins/weft-ask.js` 和 merge `opencode.json`。生成文件会尽量写入 worktree 的 git exclude,不写 canonical 仓配置。
...
7. **尊重 repo origin:worktree 可在仓内,接线不进 tracked 配置**(见 2.1)。工作目录按目标 repo 习惯放在 repo-local `.worktrees/weft/<branch>`，但跨仓挂载与会话配置只通过临时参数/一次性 worktree 注入,不写进受版本管理的仓库配置,保证单仓独立、工作区互不污染、零累积。
8. **产品化:屏蔽机制,呈现决策与结果**(见 4.7)。隐藏 plumbing(worktree/headless 进程/MCP/add-dir/旁路),用产品词表达;但用户负责的"决策与结果"(scope、repo-native 分支/PR/合并/上线、工具选择)留在台前。隐藏三件套:抽象 + 逃生舱(真路径/开终端)+ 失败可读。
9. **多语言(中/英)从第一天就内建**(见 4.8)。UI 文案外置 + agent 产出语言两层都要,绝不硬编码字符串。
10. **Automation-first,Weft 不自加审批关**。产品北极星是自动化:lead 默认自动分解→spawn→派发→驱动到实现、PR、合并和上线。**唯一的阻塞性人工来自工具自身的权限/审批习惯**(Codex/Claude 按用户自己的配置弹出),Weft 只透传(见 4.3),不新增 gate。人是监督/随时介入/异常处理,不是必经关卡。仅"不可逆/爆炸半径大"的动作(合并受保护分支、生产发布、破坏性/资金操作)留**可配置**边界,默认由用户定;git/CI/工具权限已是安全网。
11. **交付边界分阶段推进**。当前代码已经到 repo-native worktree diff + pre-PR checks;下一产品边界是 Task → PR。merge / CI-CD / release 先接入仓里现有 harness，再由 Weft 逐步编排。因 Weft 驱动原生 CLI(不绕 hooks),worker 后续开/更新 PR 时**仓库自身的 PR 触发 hooks/CI 自然触发**——交付即自动接上现有 PR harness,接缝在 PR。CI/CD 反应式、更高维 harness 为未来维度(架构经 git/forge+bus 已可容纳),但北极星是需求一路做到合并和上线。
...
├── <repo>/.worktrees/weft/<branch>/      # repo-local git worktree
│     ├── (agent 注入文件, 全部 git exclude)
│     └── (该 direction 的真实代码改动)
├── branch                               # 遵循目标 repo 风格: feat/* vs feature/*, fix/* vs bugfix/*
...
### 2.1 关键隔离原则:repo-local worktree, no tracked wiring

Weft 尊重目标仓库自己的 origin：工作目录可以放在该仓库下的 `.worktrees/weft/<branch>`，
分支名也按该仓库历史风格生成。但跨仓挂载(add-dir)和会话级配置有"临时"与"持久"两种形态，**编排器只用临时形态**，否则会把 workspace wiring 写进仓库历史、让单仓使用负重前行:

| 工具 | 临时形态(用这个) | 持久形态(别写进 tracked 仓配置) |
|---|---|---|
| Claude Code | `--add-dir <path>`(启动 flag)、`/add-dir`(会话内)——仅当前会话,不落盘 | `additionalDirectories` 写进仓的 `.claude/settings.json`(提交后永久跟随该仓) |
| Codex | `--add-dir` / `-C` / `-c key=val`(启动参数)——临时；Ask Bridge 走 stable global hook + worktree-local route 文件 | `writable_roots` 写进仓的 `config.toml`，或动态 per-session hook 写进用户根配置 |

**规则**:工作区的跨仓接线与会话级配置,只活在 **(a) 临时启动参数 / `--settings` / env**,或 **(b) repo-local worktree 内的 git-excluded 文件**(随 worktree 用完即删)。**永远不进目标 repo 的受版本管理配置。**

由此得到三条保证:

1. **单独改一个仓**:直接打开 canonical checkout,只加载该仓自身 + 用户配置,看不到任何工作区接线 → 干净。
2. **推进 workspace B**:B 物化自己的 repo-local worktree + 自己的启动参数,不继承 A 的任何接线。
3. **零 tracked 累积**:Weft 文件在 `.worktrees/weft` 或 git exclude 中,不会进入仓库历史。
...
5. **当前收尾边界**:按 worktree 看 diff → 跑 pre-PR checks / review skill → 人 review。下一产品边界是自动开/更新 PR；北极星继续覆盖合并与上线。
...
- **同仓多 worktree**:一个仓被多个"thread×方向"同时写时各派生一个 repo-local worktree(共享对象库,零拷贝),路径为 `<repo>/.worktrees/weft/<branch>`；产品做命名空间管理 + 控制磁盘/依赖成本(每 worktree 一份依赖 → 懒装/链接共享)。
